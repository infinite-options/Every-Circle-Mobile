...
const PROFILE_ENDPOINT = "https://ioec2testsspm.infiniteoptions.com/userprofileinfo";
...
export default function LoginScreen({ navigation, onGoogleSignIn, onAppleSignIn, onError }) {
  ...
  const handleContinue = async () => {
    try {
      ...
      console.log("--- debug --- loginObject:", loginObject);
      console.log("--- debug --- loginObject.result:", loginObject.message);
      console.log("--- debug --- loginObject.result.user_uid:", loginObject.result.user_uid);
      ...
      try {
        ...
        if (!fullUser || fullUser.message === "Profile not found for this user") {
          // If no profile exists, navigate to UserInfo screen
          navigation.navigate("UserInfo");
          return;
        }

        // If profile exists, navigate to Profile screen
        ...
        // If there's an error fetching the profile, navigate to UserInfo screen
        navigation.navigate("UserInfo");
      }
    } catch (error) {
