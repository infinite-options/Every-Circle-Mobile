...
export default function SearchScreen({ route }) {
  ...
  const onSearch = async () => {
    ...
    try {
      ...
      const apiUrl = `${API_BASE_URL}/api/businessresults/${encodeURIComponent(q)}`;
      // const apiUrl = `${API_BASE_URL}/api/v1/tagsplitsearchdistinct/${encodeURIComponent(q)}`;
      // const apiUrl = `${API_BASE_URL}/api/tagcategorydistinct/${encodeURIComponent(q)}`;
  ...
  const tryAlternativeEndpoints = async (query) => {
    const alternativeEndpoints = [
      ...
      `${API_BASE_URL}/api/tagsearchdistinct/${encodeURIComponent(query)}`,
      `${API_BASE_URL}/api/tagcategorydistinct/${encodeURIComponent(query)}`,
    ];
