...
const MiniCard = ({ user, business, type = 'user' }) => {
  const data = type === 'user' ? user : business;

  const firstName = type === 'user'
    ? data?.personal_info?.profile_personal_first_name || data?.firstName || ''
    : data?.business_name || '';

  const lastName = type === 'user'
    ? data?.personal_info?.profile_personal_last_name || data?.lastName || ''
    : '';

  const tagLine = data?.tagLine || data?.business_tagline || '';
  const email = data?.email || data?.business_email || '';
  const phone = data?.phone || data?.business_phone_number || '';

  const emailIsPublic = type === 'user'
    ? data?.personal_info?.profile_personal_email_is_public === 1 || data?.emailIsPublic
    : String(data?.business_email_id_is_public) === '1' || data?.emailIsPublic;

  const phoneIsPublic = type === 'user'
    ? data?.personal_info?.profile_personal_phone_number_is_public === 1 || data?.phoneIsPublic
    : String(data?.business_phone_number_is_public) === '1' || data?.phoneIsPublic;

  const tagLineIsPublic = type === 'user'
    ? data?.personal_info?.profile_personal_tagline_is_public === 1 || data?.tagLineIsPublic
    : String(data?.business_tag_line_is_public) === '1' || data?.taglineIsPublic;
